"""add server-backed Hold/Resume to the Retail boundary

Revision ID: p12retail0006
Revises: p11retail0005
"""
...
revision: str = "p12retail0006"
down_revision: Union[str, None] = "p11retail0005"
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint("company_id", "branch_id", "idempotency_key", name="uq_pos_hold_drafts_idempotency"),
    )
    for name, columns in (
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("company_id", "branch_id", "action", "idempotency_key", name="uq_pos_hold_draft_audits_idempotency"),
    )
    for name, columns in (
    ...
    op.execute("""
        CREATE FUNCTION prevent_retail_pos_hold_draft_audit_mutation()
        RETURNS trigger LANGUAGE plpgsql AS $$
        BEGIN RAISE EXCEPTION 'pos_hold_draft_audits is append-only' USING ERRCODE = '55000'; END; $$
    """)
    op.execute("""
        CREATE TRIGGER trg_retail_pos_hold_draft_audits_append_only
        BEFORE UPDATE OR DELETE ON pos_hold_draft_audits
        FOR EACH ROW EXECUTE FUNCTION prevent_retail_pos_hold_draft_audit_mutation()
    """)


def downgrade() -> None:
    op.execute("DROP TRIGGER IF EXISTS trg_retail_pos_hold_draft_audits_append_only ON pos_hold_draft_audits")
    op.execute("DROP FUNCTION IF EXISTS prevent_retail_pos_hold_draft_audit_mutation()")
