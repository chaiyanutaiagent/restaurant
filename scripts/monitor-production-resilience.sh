...
python3 -c '
import json, sys
alerts = [line.rstrip("\n") for line in open(sys.argv[10], encoding="utf-8") if line.strip()]
payload = {
...
with open(sys.argv[11], "w", encoding="utf-8") as handle:
    ...
    handle.write("\n")
' "$timestamp" "$STATUS" "$health_code" "$projector_failed" "$projector_loop_errors" "$disk_percent" "$latest_backup" "$backup_age_hours" "$([ -n "$ALERT_WEBHOOK_URL" ] && printf true || printf false)" "$ALERTS_FILE" "$EVIDENCE_FILE"
