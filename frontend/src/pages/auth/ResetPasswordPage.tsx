...
export default function ResetPasswordPage(): JSX.Element {
  ...
  const loginPath = businessSlug ? `/${businessSlug}/login` : "/login";
  ...
  return <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4"><Card className="w-full max-w-md"><CardHeader><CardTitle>ตั้งรหัสผ่านใหม่</CardTitle></CardHeader><CardContent>{done ? <div className="space-y-4 text-center"><p className="text-emerald-700">เปลี่ยนรหัสผ่านแล้ว และ session เดิมถูกเพิกถอน</p><Link className="text-blue-600" to={loginPath}>เข้าสู่ระบบ</Link></div> : <form className="space-y-4" onSubmit={submit}><div className="space-y-2"><Label htmlFor="new-password">รหัสผ่านใหม่อย่างน้อย 12 ตัว</Label><Input id="new-password" type="password" minLength={12} required value={password} onChange={(e) => setPassword(e.target.value)} /></div>{error ? <p className="text-sm text-red-600">{error}</p> : null}<Button className="w-full" disabled={!token} type="submit">บันทึกรหัสผ่านใหม่</Button></form>}</CardContent></Card></div>;
}
