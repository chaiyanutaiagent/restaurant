...
revision: str = "p8member0010"
down_revision: Union[str, None] = "p7usage0009"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "onboarding_state",
            sa.String(length=30),
            server_default=sa.text("'awaiting_verification'"),
            nullable=False,
        ),
        ...
        sa.CheckConstraint(
            ...
            name="ck_saas_tenant_memberships_status_valid",
        ),
        sa.CheckConstraint(
            "onboarding_state IN ('awaiting_verification', 'setup_required', 'ready')",
            name="ck_saas_tenant_memberships_onboarding_state_valid",
        ),
    ...
    op.create_index(
        "ix_saas_memberships_status_trial",
        "saas_tenant_memberships",
        ["status", "trial_ends_at"],
    )
    op.create_index(
        "ix_saas_tenant_memberships_trial_ends_at",
        "saas_tenant_memberships",
        ["trial_ends_at"],
    )
    op.create_table(
        ...
        sa.CheckConstraint(
            ...
            name="ck_saas_account_credentials_purpose_valid",
        ),
        sa.ForeignKeyConstraint(["company_id"], ["companies.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["owner_user_id"], ["users.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_index(
        "ix_saas_account_credentials_company",
        "saas_account_credentials",
