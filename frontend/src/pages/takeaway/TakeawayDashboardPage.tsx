      ...
      <section className="overflow-hidden rounded-3xl bg-slate-950 p-6 text-white shadow-xl md:p-8">
        <div className="flex flex-wrap items-center justify-between gap-5">
          ...
          <div className="rounded-2xl border border-slate-700 bg-slate-900 px-5 py-4">
            ...
            <p className="mt-1 flex items-center gap-2 font-bold"><span className={`h-2.5 w-2.5 rounded-full ${context?.enabled ? "bg-emerald-400" : "bg-amber-400"}`} />{statusQuery.isLoading ? "กำลังตรวจ" : context?.enabled ? "พร้อมใช้งาน" : "ปิดไว้เพื่อความปลอดภัย"}</p>
          </div>
        </div>
      </section>
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {cards.map((card) => (
          <Link key={card.to} to={card.to} className="group rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
            ...
            <span className="mt-4 flex items-center gap-1 text-sm font-bold text-emerald-700">เปิดใช้งาน <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" /></span>
          </Link>
        ))}
      </section>
