"""add WP50 shift compatibility to the Retail boundary

Revision ID: p9retail0003
Revises: p8retail0002
"""
...
revision: str = "p9retail0003"
down_revision: Union[str, None] = "p8retail0002"
...
def upgrade() -> None:
    ...
    op.add_column("cashier_shifts", sa.Column("cash_count_json", sa.JSON(), nullable=True))
    op.add_column("cashier_shifts", sa.Column("opened_device_id", sa.UUID(), nullable=True))
    op.add_column("cashier_shifts", sa.Column("opened_device_code", sa.String(length=100), nullable=True))
    op.add_column("cashier_shifts", sa.Column("closed_device_id", sa.UUID(), nullable=True))
    op.add_column("cashier_shifts", sa.Column("closed_device_code", sa.String(length=100), nullable=True))
    op.add_column("cashier_shifts", sa.Column("closed_by_user_id", sa.UUID(), nullable=True))
    op.add_column("cashier_shifts", sa.Column("approval_evidence", sa.JSON(), nullable=True))
    op.add_column("cashier_shifts", sa.Column("close_snapshot_json", sa.JSON(), nullable=True))
    ...
    op.create_index("uq_cashier_shifts_open_idempotency", "cashier_shifts", ["company_id", "branch_id", "open_idempotency_key"], unique=True, postgresql_where=sa.text("open_idempotency_key IS NOT NULL"))
    op.create_index("uq_cashier_shifts_close_idempotency", "cashier_shifts", ["company_id", "branch_id", "close_idempotency_key"], unique=True, postgresql_where=sa.text("close_idempotency_key IS NOT NULL"))
    ...
    op.create_table(
        ...
        sa.Column("approval_evidence", sa.JSON(), nullable=True),
...
def downgrade() -> None:
    ...
    for column in (
        ...
        "opened_device_code", "opened_device_id", "cash_count_json", "close_reason_code", "close_request_hash",
        "close_idempotency_key", "open_request_hash", "open_idempotency_key", "version", "shift_type",
    ):
