...
cleanup() {
  ...
  if [[ -n "$TEMP_DIR" ]]; then
    case "$TEMP_DIR" in
      /private/tmp/restaurant-p3-workspace.*|/tmp/restaurant-p3-workspace.*) rm -rf "$TEMP_DIR" ;;
    esac
  fi
}
...
[[ "$(psql_boundary POSTGRES_DB 'SELECT version_num FROM alembic_version')" = "$EXPECTED_LEGACY_HEAD" ]] || fail "unexpected legacy source head"
[[ "$(psql_boundary PLATFORM_POSTGRES_DB 'SELECT version_num FROM alembic_version')" = "$EXPECTED_PLATFORM_HEAD" ]] || fail "unexpected Platform source head"
[[ "$(psql_boundary RESTAURANT_POSTGRES_DB 'SELECT version_num FROM alembic_version')" = "$EXPECTED_RESTAURANT_HEAD" ]] || fail "unexpected Restaurant source head"

timestamp="$(date -u +%Y%m%dT%H%M%SZ)"
suffix="$(date -u +%Y%m%d%H%M%S)$$"
LEGACY_DATABASE="${LEGACY_PREFIX}${suffix}"
PLATFORM_DATABASE="${PLATFORM_PREFIX}${suffix}"
artifact_dir="${ARTIFACT_ROOT%/}/p3-dedicated-workspaces-02-${timestamp}"
mkdir -p "$artifact_dir"
TEMP_DIR="$(mktemp -d /private/tmp/restaurant-p3-workspace.XXXXXX 2>/dev/null || mktemp -d /tmp/restaurant-p3-workspace.XXXXXX)"
...
printf 'Building current backend image and applying existing Phase 3 device migrations to clones...\n'
docker compose -f "$COMPOSE_FILE" build backend >/dev/null
docker compose -f "$COMPOSE_FILE" run --rm --no-deps -e P3_WORKSPACE_DATABASE_NAME="$LEGACY_DATABASE" backend sh -c '
  set -eu
...
printf 'Running dedicated Counter, Kitchen, and Pickup API gate...\n'
smoke_output=""
if ! smoke_output="$(docker compose -f "$COMPOSE_FILE" run --rm --no-deps \
  -e P3_WORKSPACE_DATABASE_NAME="$LEGACY_DATABASE" \
  -e P3_WORKSPACE_PLATFORM_DATABASE_NAME="$PLATFORM_DATABASE" \
  ...
  backend sh -c '
    ...
    python /app/tests/smoke_device_workspaces_api.py
  ' 2>&1)"; then
  printf '%s\n' "$smoke_output"
  fail "dedicated device workspace API gate failed"
fi
printf '%s\n' "$smoke_output"
printf '%s\n' "$smoke_output" | grep -F 'p3_device_workspaces_api=ok' >/dev/null || fail "workspace smoke assertion is missing"
...
printf 'Phase 3 dedicated workspace rehearsal passed: %s\n' "$artifact_dir"
