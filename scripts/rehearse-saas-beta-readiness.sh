...
export RESTAURANT_BACKEND_IMAGE="${SAAS_BETA_BACKEND_IMAGE:-restaurant-pos-dev-backend:saas-beta-readiness-gate}"
ARTIFACT_ROOT="${SAAS_BETA_ARTIFACT_ROOT:-/private/tmp/restaurant-saas-artifacts}"
...
LEGACY_PREFIX="restaurant_saas_beta_"
PLATFORM_PREFIX="restaurant_saas_beta_platform_"
OPS_PREFIX="restaurant_saas_beta_ops_"
ASSUME_YES=0
LEGACY_DATABASE=""
PLATFORM_DATABASE=""
OPS_DATABASE=""
...
cleanup() {
  ...
  if [[ -n "$TEMP_DIR" ]]; then
    case "$TEMP_DIR" in
      /private/tmp/restaurant-saas-beta-readiness.*|/tmp/restaurant-saas-beta-readiness.*) rm -rf "$TEMP_DIR" ;;
    esac
  fi
}
...
printf 'Starting isolated SaaS beta readiness gate...\n'
...
artifact_dir="${ARTIFACT_ROOT%/}/saas-beta-readiness-08-${timestamp}"
backup_root="$artifact_dir/backups"
mkdir -p "$artifact_dir" "$backup_root"
TEMP_DIR="$(mktemp -d /private/tmp/restaurant-saas-beta-readiness.XXXXXX 2>/dev/null || mktemp -d /tmp/restaurant-saas-beta-readiness.XXXXXX)"
...
printf 'Building isolated beta-readiness backend image...\n'
docker compose -f "$COMPOSE_FILE" build backend >/dev/null

printf 'Rehearsing latest SaaS preparation migrations...\n'
docker compose -f "$COMPOSE_FILE" run --rm --no-deps -e SAAS_BETA_DATABASE_NAME="$LEGACY_DATABASE" backend sh -c '
  set -eu; database_server="${DATABASE_URL%/*}"; export DATABASE_URL="$database_server/$SAAS_BETA_DATABASE_NAME"
  alembic upgrade p11privacy0013; alembic downgrade p10bill0012; alembic upgrade p11privacy0013
'
docker compose -f "$COMPOSE_FILE" run --rm --no-deps -e SAAS_BETA_PLATFORM_DATABASE_NAME="$PLATFORM_DATABASE" backend sh -c '
  set -eu; database_server="${PLATFORM_DATABASE_URL%/*}"; export PLATFORM_DATABASE_URL="$database_server/$SAAS_BETA_PLATFORM_DATABASE_NAME"
  ...
  alembic -c alembic-boundaries.ini -n platform upgrade p11platform0015
'
[[ "$(psql_database "$LEGACY_DATABASE" 'SELECT version_num FROM alembic_version')" = "$TARGET_LEGACY_HEAD" ]] || fail "legacy clone did not reach latest SaaS head"
[[ "$(psql_database "$PLATFORM_DATABASE" 'SELECT version_num FROM alembic_version')" = "$TARGET_PLATFORM_HEAD" ]] || fail "Platform clone did not reach latest SaaS head"
[[ "$(psql_database "$OPS_DATABASE" 'SELECT version_num FROM alembic_version')" = "$EXPECTED_RESTAURANT_HEAD" ]] || fail "Restaurant clone migration head changed"

printf 'Running dependency consistency, static boundaries, and full backend tests...\n'
scripts/check-saas-beta-static.sh | tee "$artifact_dir/static-boundary.log"
docker compose -f "$COMPOSE_FILE" run --rm --no-deps backend python -m pip check | tee "$artifact_dir/python-dependency-check.log"
...
printf 'Running latest-schema authorization and isolation smoke...\n'
docker compose -f "$COMPOSE_FILE" run --rm --no-deps \
  -e SAAS_BETA_DATABASE_NAME="$LEGACY_DATABASE" \
...
printf 'Running bounded concurrent aggregate-endpoint load probe...\n'
docker compose -f "$COMPOSE_FILE" run --rm --no-deps \
  -e SAAS_BETA_DATABASE_NAME="$LEGACY_DATABASE" \
  -e IDENTITY_DATABASE=legacy -e RESTAURANT_SERVICE_DATABASE=legacy -e REFERENCE_PROJECTOR_ENABLED=false \
  -v "$PWD/backend/tests:/app/tests:ro" backend sh -c '
    set -eu; database_server="${DATABASE_URL%/*}"; export DATABASE_URL="$database_server/$SAAS_BETA_DATABASE_NAME"
    export PLATFORM_DATABASE_URL="$DATABASE_URL" RESTAURANT_DATABASE_URL="$DATABASE_URL"
    python -m tests.smoke_saas_beta_load
  ' | tee "$artifact_dir/load-probe.json"
...
backup_output="$(P5_LEGACY_DATABASE_NAME="$LEGACY_DATABASE" P5_PLATFORM_DATABASE_NAME="$PLATFORM_DATABASE" P5_RESTAURANT_DATABASE_NAME="$OPS_DATABASE" P5_TENANT_BACKUP_ROOT="$backup_root" scripts/backup-tenant-boundaries.sh --company-id "$company_id" --reason "SaaS beta readiness isolated recovery gate")"
printf '%s\n' "$backup_output"
...
printf 'Collecting prior Scope manifests...\n'
for scope_pattern in \
  'saas-platform-auth-02-*' \
  'saas-tenant-usage-03-*' \
  'saas-membership-04-*' \
  'saas-operations-05-*' \
  'saas-billing-06-*' \
  'saas-privacy-support-07-*'; do
  prior_manifest="$(find "$ARTIFACT_ROOT" -maxdepth 2 -type f -path "*/$scope_pattern/manifest.txt" | sort | tail -n 1)"
  [[ -n "$prior_manifest" ]] || fail "prior Scope manifest is missing for $scope_pattern"
  printf '%s\n' "$prior_manifest" >> "$artifact_dir/prior-manifests.txt"
done
...
python_dependency_consistency=passed
static_boundary_and_secret_marker_gate=passed
operations_api_smoke=passed
latest_schema_multi_tenant_api_smoke=passed
concurrent_load_probe=$artifact_dir/load-probe.json
three_boundary_backup=$backup_dir
isolated_restore_drill=passed
scheduler_evidence_import=passed
prior_scope_manifests=$artifact_dir/prior-manifests.txt
frontend_evidence=recorded_separately_in_beta_readiness_report
online_dependency_advisory_check=requires_explicit_owner_authorization
live_sources_unchanged=true
temporary_database_cleanup=automatic
EOF
chmod 600 "$artifact_dir/manifest.txt" "$artifact_dir/resilience.json" "$artifact_dir/scheduler-import.json" "$artifact_dir/prior-manifests.txt" "$artifact_dir/load-probe.json"
printf 'SaaS beta readiness gate passed. Artifact: %s\n' "$artifact_dir/manifest.txt"
