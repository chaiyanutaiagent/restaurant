...
export default function TakeawayStoreCentralOrdersPage(): JSX.Element {
  ...
  const receiveMutation = useMutation({
    mutationFn: (orderId: string) => takeawayApi.receiveStoreCentralOrder(orderId),
    onSuccess: async () => {
      await Promise.all([
  ...
  return <div className="space-y-5">
    ...
    <section className="space-y-3">
      ...
      {ordersQuery.isLoading ? <div className="flex justify-center p-10"><Loader2 className="h-7 w-7 animate-spin text-emerald-600" /></div> : orders.length ? orders.map((order) => <article key={order.id} className="rounded-2xl border bg-white p-4 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-black">{order.order_number}</p><p className="mt-1 text-xs text-slate-500">{typeLabels[order.order_type]} · ต้องการรับ {order.requested_delivery_date ?? "ไม่ระบุ"}</p></div><div className="flex items-center gap-2"><span className={`rounded-full px-3 py-1 text-xs font-bold ${order.status === "received" ? "bg-emerald-100 text-emerald-800" : order.status === "rejected" ? "bg-rose-100 text-rose-800" : "bg-amber-100 text-amber-800"}`}>{statusLabels[order.status] ?? order.status}</span>{order.status === "shipped" ? <button disabled={receiveMutation.isPending} onClick={() => receiveMutation.mutate(order.id)} className="flex items-center gap-2 rounded-xl bg-emerald-500 px-3 py-2 text-sm font-black"><PackageCheck className="h-4 w-4" /> รับของครบ</button> : null}</div></div><div className="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">{(order.items ?? []).map((item) => <div key={item.id} className="rounded-xl bg-slate-50 p-3 text-sm"><p className="font-bold">{item.item_name}</p><p className="text-xs text-slate-500">{item.quantity} {item.unit} · {item.source_kind}</p></div>)}</div>{order.status === "received" ? <p className="mt-3 flex items-center gap-2 text-sm font-bold text-emerald-700"><CheckCircle2 className="h-4 w-4" /> รับเข้าสต๊อกร้านแล้ว</p> : null}</article>) : <div className="rounded-2xl border border-dashed bg-white p-10 text-center text-slate-500">ยังไม่มีใบสั่งของสาขา</div>}
    </section>
  </div>;
}
