...
export RESTAURANT_BACKEND_IMAGE="${SAAS_AUTH_BACKEND_IMAGE:-restaurant-pos-dev-backend:saas-auth-gate}"
ARTIFACT_ROOT="${SAAS_AUTH_ARTIFACT_ROOT:-/private/tmp/restaurant-saas-artifacts}"
PROJECT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
EXPECTED_LEGACY_HEAD="6b7c8d9e0f12"
EXPECTED_PLATFORM_HEAD="p1platform0003"
TARGET_LEGACY_HEAD="p6auth0008"
TARGET_PLATFORM_HEAD="p6platform0010"
LEGACY_PREFIX="restaurant_saas_auth_"
PLATFORM_PREFIX="restaurant_saas_auth_platform_"
ASSUME_YES=0
LEGACY_DATABASE=""
PLATFORM_DATABASE=""
TEMP_DIR=""
...
cleanup() {
  ...
  if [[ -n "$TEMP_DIR" ]]; then
    case "$TEMP_DIR" in
      /private/tmp/restaurant-saas-auth.*|/tmp/restaurant-saas-auth.*) rm -rf "$TEMP_DIR" ;;
    esac
  fi
}
...
printf 'Starting isolated SaaS Platform auth gate...\n'
...
artifact_dir="${ARTIFACT_ROOT%/}/saas-platform-auth-02-${timestamp}"
mkdir -p "$artifact_dir"
TEMP_DIR="$(mktemp -d /private/tmp/restaurant-saas-auth.XXXXXX 2>/dev/null || mktemp -d /tmp/restaurant-saas-auth.XXXXXX)"
...
printf 'Building isolated SaaS auth backend image...\n'
docker compose -f "$COMPOSE_FILE" build backend >/dev/null

printf 'Rehearsing legacy auth upgrade/downgrade/re-upgrade...\n'
docker compose -f "$COMPOSE_FILE" run --rm --no-deps -e SAAS_AUTH_DATABASE_NAME="$LEGACY_DATABASE" backend sh -c '
  set -eu
  case "$SAAS_AUTH_DATABASE_NAME" in restaurant_saas_auth_[0-9]*) ;; *) exit 2 ;; esac
  database_server="${DATABASE_URL%/*}"
  export DATABASE_URL="$database_server/$SAAS_AUTH_DATABASE_NAME"
  alembic upgrade p6auth0008
  alembic downgrade p5tenant0007
  alembic upgrade p6auth0008
'

printf 'Rehearsing Platform-core auth upgrade/downgrade/re-upgrade...\n'
docker compose -f "$COMPOSE_FILE" run --rm --no-deps -e SAAS_AUTH_PLATFORM_DATABASE_NAME="$PLATFORM_DATABASE" backend sh -c '
  set -eu
  case "$SAAS_AUTH_PLATFORM_DATABASE_NAME" in restaurant_saas_auth_platform_[0-9]*) ;; *) exit 2 ;; esac
  database_server="${PLATFORM_DATABASE_URL%/*}"
  export PLATFORM_DATABASE_URL="$database_server/$SAAS_AUTH_PLATFORM_DATABASE_NAME"
  alembic -c alembic-boundaries.ini -n platform upgrade p6platform0010
  alembic -c alembic-boundaries.ini -n platform downgrade p5platform0009
  alembic -c alembic-boundaries.ini -n platform upgrade p6platform0010
'

[[ "$(psql_database "$LEGACY_DATABASE" 'SELECT version_num FROM alembic_version')" = "$TARGET_LEGACY_HEAD" ]] || fail "legacy clone did not reach SaaS auth head"
[[ "$(psql_database "$PLATFORM_DATABASE" 'SELECT version_num FROM alembic_version')" = "$TARGET_PLATFORM_HEAD" ]] || fail "Platform clone did not reach SaaS auth head"
for database_name in "$LEGACY_DATABASE" "$PLATFORM_DATABASE"; do
  [[ "$(psql_database "$database_name" "SELECT count(*) FROM information_schema.tables WHERE table_name = 'platform_sessions'")" = "1" ]] || fail "Platform sessions table is missing from $database_name"
  [[ "$(psql_database "$database_name" "SELECT count(*) FROM information_schema.columns WHERE table_name = 'platform_operators' AND column_name IN ('mfa_secret_ciphertext', 'mfa_enabled_at', 'mfa_last_verified_step', 'mfa_recovery_code_hashes')")" = "4" ]] || fail "Platform MFA columns are missing from $database_name"
done

printf 'Running focused Platform auth tests...\n'
...
printf 'Running isolated Platform auth API smoke...\n'
docker compose -f "$COMPOSE_FILE" run --rm --no-deps \
  -e SAAS_AUTH_DATABASE_NAME="$LEGACY_DATABASE" \
  ...
  backend sh -c '
    ...
    python -m tests.smoke_platform_auth_api
  '
...
scope=SAAS-PREP-PLATFORM-AUTH-02
timestamp=$timestamp
...
platform_auth_api_smoke=passed
mfa_and_recovery_gate=passed
session_rotation_and_revocation_gate=passed
live_sources_unchanged=true
temporary_database_cleanup=automatic
EOF

printf 'SaaS Platform auth gate passed. Artifact: %s\n' "$artifact_dir/manifest.txt"
